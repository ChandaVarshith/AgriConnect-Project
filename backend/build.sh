...
MODEL_FILE="plant_disease_recog_model_pwp.tflite"
if [ -f "$MODEL_FILE" ]; then
  echo "==> Model already exists, skipping download..."
else
  if [ -n "$TFLITE_GDRIVE_FILE_ID" ]; then
    echo "==> Downloading ML model from Google Drive (file ID: $TFLITE_GDRIVE_FILE_ID)..."
    "$CONDA_DIR/bin/gdown" "https://drive.google.com/uc?id=$TFLITE_GDRIVE_FILE_ID" -O "$MODEL_FILE"
    echo "==> Model downloaded: $(ls -lh $MODEL_FILE | awk '{print $5}')"
  else
    echo "==> WARNING: TFLITE_GDRIVE_FILE_ID not set, skipping model download"
  fi
fi
