...
const GeminiPanel = ({ query, cropType, location }) => {
    ...
    const handleAsk = async () => {
        ...
        setResponse('')
        try {
            const text = await geminiService.askFarmingQuery(query, cropType, location)
            setResponse(text)
        } catch {
            setError('Gemini AI is unavailable. Please check your API key.')
        } finally {
            setLoading(false)
        }
    }
    ...
    return (
        <div style={{ marginTop: 12 }}>
            <button className="btn btn-outline" onClick={handleAsk} disabled={loading}>
                {loading ? '🤖 Thinking…' : '✨ Get AI Suggestion from Gemini'}
            </button>
            ...
            {response && (
                <div style={{
                    ...
                    <p style={{ color: 'var(--accent)', fontWeight: 600, marginBottom: 6 }}>🤖 Gemini AI Suggests:</p>
